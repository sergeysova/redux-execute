...
const getStore = (options = {}) =>
  createStore(
    defaultReducer,
    { value: 0 },
    applyMiddleware(createExecute(options)),
  )
...
test("must return promise in async effect", async (t) => {
  ...
  const effect = (a, b) => async () => (await a) + (await b)
...
test("test with logging", (t) => {
  ...
  const effect = (a, b, c) => () => a + b + c
  const mockedLog = (...args) => {
    t.deepEqual(["Effect:", "effect", "Arguments:", [1, 2, 3]], args)
  }
